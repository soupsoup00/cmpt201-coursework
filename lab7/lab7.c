...
void map(Input *input, IntermediateInput *intermediate_input) {
  ...
  // processes a list of key-value pair to generate a list of intermediate
  // key-value pairs.
...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  // TODO:
  // Group intermediate key value pairs together (Values not keys)
  // If the doubled value exists in the output entries, add it to the output's
  // line_numbers list If it doesn't exist, add a new entry to the output 10,
  // [1,2,4]
  // result count = num of structs in output arr
  for (int i = 0; i < *result_count; i++) {
...
void reduce(Output *output) {
  ...
  printf("doubled value: %d, [", output->doubled_value);
